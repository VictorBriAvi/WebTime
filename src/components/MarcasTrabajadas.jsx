...
const MarcasTrabajadas = () => {
  ...
  const arregloMarcasTrabajadas = marcasTrabajadas.filter((img) => img.type==="m")
  return (
    <div className="contenedor-marcas mt-5">
      <div className="marcas-cinta my-5">
     { arregloMarcasTrabajadas.map((imagen,index) => (

      <img key={index} className="logitoMarca" src = {`/../../public/logosMarcas/${imagen.type}-${imagen.id}.png`} alt = "" / >))}
      </div>
    </div>
  );
};
